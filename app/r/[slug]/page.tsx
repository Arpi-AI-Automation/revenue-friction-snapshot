...
export default async function ReportPage({
  ...
  const { domain, generatedAt, platform, status, buckets, topFixes, compositeScore } = report
              ...
              {compositeScore != null && (
                <div className="mb-5 flex items-baseline gap-3">
                  <span className="font-serif text-xl text-primary">
                    {compositeScore}
                    <span className="text-muted text-md">/100</span>
                  </span>
                  <span className={`text-sm font-medium ${
                    compositeScore >= 70 ? 'text-friction-low'
                    : compositeScore >= 45 ? 'text-friction-mid'
                    : 'text-friction-high'
                  }`}>
                    {compositeScore >= 70 ? 'Low Friction'
                     : compositeScore >= 45 ? 'Moderate Friction'
                     : 'High Friction'}
                  </span>
                  <span className="text-2xs text-muted font-mono">overall friction score</span>
                </div>
              )}
